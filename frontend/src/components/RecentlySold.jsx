  ...
  const contractToRead = ConnectedContractRead();
    ...
    async function callForRecentlySold(){
       ...
       const response = await axios.post(
          'https://deep-index.moralis.io/api/v2/0x84EDAf725e649D0034348f345eBAA05624566861/events?chain=fantom&from_block=55587828&topic=0xa574c741071bb18f51fa88c17aeefec514bebf670ca22a3736fba6504ecbb763',
          data,
          { headers }
        )
...
<h1 className='font-light text-4xl mb-8'>Recently Sold</h1>
  {
    ...
    <Carousel responsive={responsive} >
     ...
     {metadataArr.map(card => (
      <LandingPageNftEvent image={card.image} title={card.title} description={card.description} amount={card.amount} id={card.id} address={card.buyer} isSold={true} created={false} />
     ))}
  </Carousel>
   : <LoadingComponent />
}
